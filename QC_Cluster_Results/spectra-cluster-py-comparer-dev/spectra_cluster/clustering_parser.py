...
class ClusteringParser:
    ...
    def _parse_ptms(ptm_string):
        ...
        ptm_strings2 = []
        while len(ptm_string) > 0:
            if bool(re.match('\d+\-\[', ptm_string)):
                ptm_strings2.append(ptm_string[:ptm_string.index("]") + 1])
                try:
                    ptm_string = ptm_string[ptm_string.index("]") + 2].strip()
                except Exception as e:
                    break
            else:
                try:
                    ptm_strings2.append(ptm_string[:ptm_string.index(',')])
                except Exception as e:
                    break
                else:
                    pt+
                    ...
                    break
                try:
                    ptm_string = ptm_string[ptm_string.index(',') + 1:].strip()
                except Exception as e:
                    break

        ptms = list()

        for cur_ptm_string in ptm_strings2:
            fields = cur_ptm_string.split("-",1)
            if len(fields) != 2:
